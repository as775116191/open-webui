...
export const getTokenUsageConfig = async (token: string) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/configs/token_usage`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const updateTokenUsageConfig = async (token: string, config: object) => {
	let error = null;
