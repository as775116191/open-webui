...
export const updateUserTokens = async (token: string, userId: string, tokenData: {
	...
	const res = await fetch(`${WEBUI_API_BASE_URL}/users/${userId}/tokens`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const manualTokenGrant = async (token: string, userId: string, amount: number) => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/users/${userId}/tokens/grant`, {
		method: 'POST',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
